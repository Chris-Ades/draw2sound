...
class Canvas(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.domain == "frequency":
            ...
            for point in self.position:
                ...
                text_position = QPoint(x + 10, y - 10)
                painter.drawText(text_position, f"({int(round(x * self.max_x / self.width(), 0))}, {(self.height() - y)/self.mag_norm})")
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.domain == "frequency":
            x = int(round(event.x() * self.max_x / self.width(), 0))
            y = (self.height() - event.y())/self.mag_norm
            self.mouse_position_label.setText(f"({x}, {y})")
    ...
    def chooseSound(self):
        ...
        if self.domain == "frequency":
            ...
            data_points = [(int(round(x * self.max_x / self.width(), 0)), (self.height() - y)/self.mag_norm) for x, y in self.position]
